          ...
          <section className="mb-8">
            ...
            <div className="flex flex-wrap gap-2">
              <Link
                ...
                className="inline-flex items-center px-4 py-2.5 rounded-lg bg-sky-600 text-white text-sm font-medium hover:bg-sky-700 shadow-sm transition-colors"
              >
                New customer
              </Link>
              <Link
                href="/app/leads/new"
                className="inline-flex items-center px-4 py-2.5 rounded-lg bg-white border border-slate-200 text-slate-700 text-sm font-medium hover:bg-slate-50 transition-colors"
              >
                New lead
              </Link>
              <Link
          ...
          <div className="grid lg:grid-cols-2 gap-6 mb-8">
            <section className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm">
              <div className="flex items-center justify-between mb-4">
                ...
                <Link href="/app/leads/pipeline" className="text-xs text-sky-600 hover:text-sky-700">
                  Open pipeline →
                </Link>
              </div>
                    ...
                    <div key={def.id} className="flex items-center gap-3 text-sm">
                      ...
                      <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${data.leadCount > 0 ? Math.min(100, (n / data.leadCount) * 100) : 0}%`,
                          }}
                        />
                      </div>
              ...
              <ul className="space-y-3 text-sm text-slate-600">
                ...
                {data.estimatesSentCount > 0 && (
                  <li className="flex gap-2">
                    ...
                    <span>
                      <Link href="/app/jobs" className="text-sky-600 hover:text-sky-700 font-medium">
                        {data.estimatesSentCount} estimate
                        {data.estimatesSentCount === 1 ? "" : "s"}
                      </Link>{" "}
          ...
          <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-6">
            <section className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
              <div className="px-5 py-3 border-b border-slate-100 flex justify-between items-center">
                ...
                <Link href="/app/leads" className="text-xs text-sky-600 hover:text-sky-700">
                  All leads
                </Link>
              </div>
            ...
            <section className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
              <div className="px-5 py-3 border-b border-slate-100 flex justify-between items-center">
                ...
                <Link href="/app/schedule" className="text-xs text-sky-600 hover:text-sky-700">
                  Calendar
                </Link>
              </div>
              <ul className="divide-y divide-slate-100">
                    ...
                    >
                      <div className="font-medium text-slate-800 text-sm">
                        {j.scheduledStartDate
                          ? formatDateShortWeekday(j.scheduledStartDate)
                          : "TBD"}
                      </div>
                      <div className="text-xs text-slate-500 mt-0.5">
                        {j.propertyLine1 || j.customerLabel}
                      </div>
            ...
            <section className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden md:col-span-2 xl:col-span-1">
              <div className="px-5 py-3 border-b border-slate-100 flex justify-between items-center">
                <h2 className="text-sm font-semibold text-slate-800">Open tasks</h2>
                <Link href="/app/tasks" className="text-xs text-sky-600 hover:text-sky-700">
                  All tasks
                </Link>
              </div>
              <ul className="divide-y divide-slate-100">
