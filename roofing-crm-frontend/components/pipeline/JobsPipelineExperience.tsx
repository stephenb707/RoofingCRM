...
export type JobsPipelineVariant = "page" | "embedded";

export default function JobsPipelineExperience({
  variant,
  customerIdFilter,
}: {
  variant: JobsPipelineVariant;
  /** When set (e.g. from job list), limits pipeline to that customer. Embedded combined view passes null. */
  ...
  const pipelineKey = queryKeys.jobsPipeline(auth.selectedTenantId, customerIdFilter);
  ...
  const jobsListHref =
    customerIdFilter != null && customerIdFilter !== ""
      ? `/app/jobs?customerId=${encodeURIComponent(customerIdFilter)}`
      : "/app/jobs";
  ...
  const loadingBlock = (
    ...
    >
      ...
      <p className="text-sm text-slate-500">Loading job pipeline…</p>
    </div>
  );
  ...
  if (defsLoading || isLoading) {
    ...
    return (
      <section data-testid="combined-pipeline-job-section">{loadingBlock}</section>
    );
  }
  ...
  if (isError) {
    const errBody = (
      <div className="bg-red-50 border border-red-200 rounded-xl p-6">
        <h3 className="text-sm font-medium text-red-800">Failed to load job pipeline</h3>
    ...
    return (
      <section data-testid="combined-pipeline-job-section">{errBody}</section>
    );
  }
  ...
  if (variant === "embedded") {
    return (
      <section
        ...
        data-testid="combined-pipeline-job-section"
      >
        <h2 className="text-lg font-semibold text-slate-800">Job pipeline</h2>
